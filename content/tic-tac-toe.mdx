...
Assembly language is considered a low-level language because it provides little to no abstraction from a computer's architecture. Moving a value into a register is further abstracted in higher-level languages such as C++ or Python. For example, the following C++ code initializes an integer value to 97.
...
The above assembly code defines a variable initialized to the hex value of 97 with the `db` (define byte) directive. Variables defined this way would correspond to global or static variables in C++.
...
Lastly, the text section or **code section** is the portion of the program that contains executable instructions like `MOV`.
...
The second macro, `printRow` is used to print a row of the tic-tac-toe board given an index. It is used inside of another macro, `printBoard`.
...
The `printBoard` macro prints the tic-tac-toe board one row at a time.
It is a good example of how to implement a loop in assembly language using labels and jumps. 

The **label** `%%printRowStart` is the beginning of the loop. The **jump** instruction `jbe %%printRowStart` is the end of the loop.

A jump instruction jumps to a specified label and `JBE` stands for jump-below-equal or jump if less-than or equal. It depends on the result of the compare instruction `CMP RCX, [tttRow]` one line above. The `RCX` register contains the number of rows printed and is compared to the max tic-tac-toe row size of 3. Therefore, `JBE` will jump to the starting label while `RCX` (rows printed) is less than 3.

Note: The `CMP` instruction can be thought of as an `if` statement in C++. A normal `JMP` instruction is unspecified, meaning it does not depend on the result of a `CMP` instruction.
...
Note that we jump to a label outside of the macro. While connections can be made between a macro in assembly and a function in C++, it is important to note the distinction between the two. When a macro is called, it simply executes the group of instructions contained inside.

A function in C++ corresponds to a procedure in assembly language, not a macro. However, like a function/procedure, a macro improves readability and reduces repetitive code. The advantage is that it does not need a stack or require extra time to call and return.

Note: Macros can be defined in C++ with the `#define` directive. However, functions are often preferred.

### Program Logic

After the macro definitions comes the actual logic of the program, where instructions are executed.
...
After prompting a user to place a symbol on the board, the program accepts user input and validates it. If the input is invalid, the program will output an error message and return to the `inputStart` label. This section shows how to get and store user input in assembly language.
...
While it is important to check that the input itself is valid, we must also check that it corresponds to a valid placement on the tic-tac-toe board. This section makes use of the `checkPlacement` macro defined above.
...
If a placement is valid the program will print a new board showing the new placement, making use of the `printBoard` macro.
